                    ...
                    <div class="bg-gradient-to-t from-[#262626]/0 to-[#666666] pt-8 px-4 laptop:flex  tablet:px-8">
                        ...
                        <div class="flex-1">
                            <div class="flex justify-between items-center mt-8  laptop:mt-8">
                                ...
                                <div class="my-4 mx-4 tablet:mx-8 flex gap-x-2 items-center justify-end">
                                    <div v-if="showStore.currentShow.apple_podcast">
                                        <a :href="showStore.currentShow.apple_podcast" target="_blank">
                                    ...
                                    <div v-if="showStore.currentShow.sound_cloud">

                                        <a :href="showStore.currentShow.sound_cloud" target="_blank">
                                    ...
                                    <div v-if="showStore.currentShow.pod_beam">
                                        <a :href="showStore.currentShow.pod_beam">
                                    ...
                                    <div v-if="showStore.currentShow.spotify">
                                        <a :href="showStore.currentShow.spotify" target="_blank">
                                    ...
                                    <div v-if="showStore.currentShow.google_podcast">
                                        <a :href="showStore.currentShow.google_podcast">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <div class="flex gap-x-1 items-center">
                                    <div>
                                        <svg width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <circle cx="4" cy="4" r="4" class="fill-[#E84344]" fill-opacity="0.4"/>
                                        </svg>
                                    </div>
                            ...
                            <p :class="[drop ? 'line-clamp-none' : 'line-clamp-4']" class="text-white/80 break-all text-base leading-7">{{ showStore.currentShow.description }}</p>
                            <span @click.stop="dropdesc" class="text-white/80 font-semibold text-x-sm">{{ drop ? 'less...' : 'more...' }}</span>

                        </div>
                    </div>

                    <div class="flex gap-x-4 my-4 mx-4 tablet:mx-8 laptop:mb-8">
                        <span :class="[currentTag === 'EpisodeListComponent' ? 'bg-[#595959]' : 'bg-[#404040]/40']" @click="currentTag = 'EpisodeListComponent'" class="px-2 py-1 laptop:cursor-pointer hover:bg-[#404040]/60 flex gap-x-1 transition items-center text-white rounded-full" type="type">
                            <div>
                                <svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path :class="[currentTag === 'EpisodeListComponent' ? 'fill-white' : 'fill-white/60']"  d="M7.0696 7.04559C7.0696 6.64362 6.82 6.31764 6.512 6.31764H5.264C5.524 4.65774 6.6496 3.40982 8 3.40982C9.3504 3.40982 10.476 4.65974 10.7352 6.31964H8.744C8.436 6.31964 8.1864 6.64462 8.1864 7.04559C8.1864 7.44757 8.436 7.77355 8.744 7.77355H10.7904V9.22746H8.744C8.436 9.22746 8.1864 9.55344 8.1864 9.95441C8.1864 10.3564 8.436 10.6824 8.744 10.6824H10.7352C10.5128 12.1073 9.652 13.2292 8.5584 13.5182V15.773C8.5584 16.174 8.308 16.5 8 16.5C7.692 16.5 7.4416 16.174 7.4416 15.773V13.5182C6.348 13.2282 5.4872 12.1073 5.2656 10.6824H6.5112C6.82 10.6824 7.0696 10.3564 7.0696 9.95441C7.0696 9.55344 6.82 9.22746 6.512 9.22746H5.2088V7.77355H6.5104C6.8192 7.77355 7.0688 7.44757 7.0688 7.04659L7.0696 7.04559Z"/>
                                    <path :class="[currentTag === 'EpisodeListComponent' ? 'fill-white' : 'fill-white/60']" d="M8 0.5C5.336 0.5 3.1568 3.20083 2.9872 6.61562C2.69837 6.41984 2.37497 6.31741 2.0464 6.31764C0.916 6.31764 0 7.51156 0 8.98447V10.9243C0 12.3963 0.916 13.5902 2.0464 13.5902C2.0848 13.5902 2.1232 13.5882 2.1608 13.5862V13.5902C3.228 13.5902 4.0928 12.4633 4.0928 11.0723V7.04459C4.0928 4.23377 5.8424 1.95491 8 1.95491C10.1576 1.95491 11.9072 4.23377 11.9072 7.04459V11.0723C11.9072 12.4623 12.7712 13.5902 13.8392 13.5902V13.5862C13.8768 13.5892 13.9152 13.5902 13.9536 13.5902C15.084 13.5902 16 12.3963 16 10.9233V8.98347C16 7.51156 15.084 6.31764 13.9536 6.31764C13.6144 6.31764 13.2944 6.42563 13.0128 6.61562C12.8432 3.20083 10.6632 0.5 8 0.5Z" />
                                </svg>
                            </div>
                            <span :class="[currentTag === 'EpisodeListComponent' ? 'text-white' : 'text-white/60']" class="text-sm  font-medium">
                                podcasts
                            </span>
                        </span>
                        <span :class="[currentTag === 'LetsTalkComponent' ? 'bg-[#595959]' : 'bg-[#404040]/40']" @click="currentTag = 'LetsTalkComponent'" class="px-2 py-1 laptop:cursor-pointer hover:bg-[#404040]/60 transition flex gap-x-1 items-center text-hite rounded-full" type="type">
                            <div>
                                <svg width="16" height="13" viewBox="0 0 16 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path :class="[currentTag === 'LetsTalkComponent' ? 'fill-white' : 'fill-white/60']" d="M7 12.5C7 12.5 6 12.5 6 11.5C6 10.5 7 7.5 11 7.5C15 7.5 16 10.5 16 11.5C16 12.5 15 12.5 15 12.5H7ZM11 6.5C11.7956 6.5 12.5587 6.18393 13.1213 5.62132C13.6839 5.05871 14 4.29565 14 3.5C14 2.70435 13.6839 1.94129 13.1213 1.37868C12.5587 0.816071 11.7956 0.5 11 0.5C10.2044 0.5 9.44129 0.816071 8.87868 1.37868C8.31607 1.94129 8 2.70435 8 3.5C8 4.29565 8.31607 5.05871 8.87868 5.62132C9.44129 6.18393 10.2044 6.5 11 6.5ZM5.216 12.5C5.06776 12.1878 4.99382 11.8455 5 11.5C5 10.145 5.68 8.75 6.936 7.78C6.30909 7.58684 5.65595 7.49237 5 7.5C1 7.5 0 10.5 0 11.5C0 12.5 1 12.5 1 12.5H5.216ZM4.5 6.5C5.16304 6.5 5.79893 6.23661 6.26777 5.76777C6.73661 5.29893 7 4.66304 7 4C7 3.33696 6.73661 2.70107 6.26777 2.23223C5.79893 1.76339 5.16304 1.5 4.5 1.5C3.83696 1.5 3.20107 1.76339 2.73223 2.23223C2.26339 2.70107 2 3.33696 2 4C2 4.66304 2.26339 5.29893 2.73223 5.76777C3.20107 6.23661 3.83696 6.5 4.5 6.5Z"/>
                                </svg>
                            </div>
                            <span :class="[currentTag === 'LetsTalkComponent' ? 'text-white' : 'text-white/60']" class="text-sm font-medium">
                                let's talk
                            </span>
                        </span>
                    </div>

                    <component :showName="showStore.currentShow.title" :is="tag[currentTag]"></component>
                    <div class="h-[100px]">
